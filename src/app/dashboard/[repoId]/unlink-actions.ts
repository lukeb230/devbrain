...
async function ownedRepo(repoId: string, formData?: FormData) {
  ...
  if (me.orgId !== repo.org_id) redirect(withError(back, "admin_only"));
  return { repo, login: me.login };
}

export async function unlinkRepo(formData: FormData): Promise<void> {
  const ctx = await ownedRepo(String(formData.get("repoId") || ""), formData);
  if (!ctx) return;
  ...
  redirect(surfaceRoot(returnTo(formData, "/dashboard")) + "?unlinked=" + encodeURIComponent(ctx.repo.full_name));
}
...
export async function deleteRepo(formData: FormData): Promise<void> {
  ...
  revalidatePath("/widget");
  revalidatePath("/desk", "layout");
  redirect(surfaceRoot(returnTo(formData, "/dashboard")) + "?deleted=" + encodeURIComponent(ctx.repo.full_name));
}
