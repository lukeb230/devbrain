...
export default async function DeskTeam({ searchParams }: { searchParams: Promise<{ error?: string; linked?: string }> }) {
  ...
  const [{ count: repoCount }, { count: memberCount }, { count: ownerCount }, { data: orgRow }, { data: usage }, { data: recentAlerts }, operator, hints] = await Promise.all([
  ...
  const numTone = pct >= 100 ? "text-stop" : pct >= 80 ? "text-wait" : "text-txt";
  const barTone = pct >= 100 ? "bg-stop" : pct >= 80 ? "bg-wait" : "bg-accent2";
        ...
        <div className="mt-6 grid grid-cols-2 gap-7">
          <div>
            ...
            <section className="mt-7 rounded-xl border border-line bg-row px-5 py-[18px]">
              <div className="flex items-baseline"><h3 className="m-0 font-display text-[18px] font-medium text-txt">AI usage today</h3><span className="ml-auto font-mono text-[10.5px] text-faint">resets 00:00 UTC</span></div>
              <div className="mt-3 flex items-baseline gap-2"><span className={`font-display text-[40px] font-medium leading-none ${numTone}`}>{calls}</span><span className="text-[13px] text-muted">of {cap} calls</span></div>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-row2"><div className={`h-full ${barTone}`} style={{ width: `${pct}%` }} /></div>
              <div className="mt-2 font-mono text-[11px] text-muted">{Number(usage?.input_tokens ?? 0).toLocaleString()} in / {Number(usage?.output_tokens ?? 0).toLocaleString()} out tokens</div>
              <p className="mt-2.5 text-[12.5px] leading-[1.6] text-muted">{pct >= 100 ? "Budget spent — reviews, journals and digests pause for this team until midnight UTC. Presence, collisions and merge lights keep running." : "Reviews, journals, spec extraction and the daily digest count. Presence and lights never do."}</p>
            </section>
