...
import { dismissAlert, sendTestAlert } from "./alert-actions";
...
export default async function OrgSettingsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ count: repoCount }, { count: memberCount }, { count: ownerCount }, { data: orgRow }, { data: usage }, { data: recentAlerts }] = await Promise.all([
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl px-6 py-6">
        ...
        <section className="card mb-6 card-pad">
          ...
          <p className="mt-1 text-sm text-slate-500">
            When something breaks for the team — a repo losing GitHub access, the AI budget running out, a sync error — owners and admins get a
            macOS notification from the DevBrain app and a banner here. Nothing to configure; turn it off per Mac under the panel&apos;s notification settings.
          </p>
          {hasRole(me.role, "admin") && (
            <form action={sendTestAlert} className="mt-3">
              <button className="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Send a test notification</button>
            </form>
          )}
            ...
            <ul className="mt-4 divide-y divide-slate-100 border-t border-slate-100 text-sm">
              {(recentAlerts ?? []).map((a) => (
                <li key={a.id} className="flex items-center gap-2 py-1.5">
                  ...
                  {!a.resolved_at && hasRole(me.role, "admin") && (
