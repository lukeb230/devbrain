...
import { completeTask, createTask } from "../dashboard/[repoId]/tasks/actions";
...
export interface WidgetData {
  ...
  tasks: { id: string; repo_id: string; repo: string; title: string; priority: number; tags: string[]; assigned_to: string | null; status: string; done_by: string | null }[];
      ...
      <div className={"min-h-0 flex-1 px-3 py-2.5 " + (tab === "Home" ? "overflow-hidden" : "overflow-y-auto")}>
        ...
        {tab === "Tasks" && (
          <div className="space-y-2.5">
            {data.lastRepo && (
              <form action={createTask} className="card flex gap-1.5 px-2.5 py-2">
                ...
                <button className="rounded bg-brand-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-brand-700">Add</button>
              </form>
            )}
            <div className="card px-2.5 py-2">
              <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Open</div>
              {open.length === 0 ? (
                <p className="text-xs text-slate-400">No open tasks.</p>
              ) : (
                <ul className="space-y-1.5">{open.map((t) => <TaskRow key={t.id} t={t} />)}</ul>
              )}
            </div>
            {done.length > 0 && (
              <div className="card px-2.5 py-2">
                <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Completed</div>
                <ul className="space-y-1">
                  {done.map((t) => (
                    <li key={t.id} className="truncate text-xs text-slate-400 line-through">
                      {t.title} <span className="no-underline">· {t.done_by}</span>
                    </li>
                  ))}
                </ul>
              </div>
