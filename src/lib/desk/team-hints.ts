...
export async function teamHints(orgId: string, userId: string, repoName: string | null): Promise<TeamHints> {
  ...
  const [{ count: members }, { count: tokens }, { data: orgRow }, { data: usage }, { count: lists }, { data: tick }] = await Promise.all([
    ...
    admin.from("orgs").select("ai_daily_cap, plan, billing_status, trial_ends_at").eq("id", orgId).single(),
  ...
  const planText = orgRow?.billing_status === "comped" ? "comped" : orgRow?.billing_status === "trialing" ? (trialDays === null ? "trial" : `trial · ${trialDays}d`) : orgRow?.billing_status === "past_due" ? "payment failed" : orgRow?.billing_status === "canceled" ? "canceled" : orgRow?.plan === "scale" ? "Scale" : "Base";
  return {
    plan: { text: planText, tone: orgRow?.billing_status === "past_due" || orgRow?.billing_status === "canceled" || (trialDays !== null && trialDays <= 2) ? "wait" : "muted" },
